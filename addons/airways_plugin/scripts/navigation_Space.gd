...
@export_subgroup("Collsion (WIP)")
## @experimental
##[b]WIP Does not work currently... yet[/b] [br]The physics layer of the node scans the environment, allowing for the exclusion of specific objects that are not considered 'terrain' Ensuring that the node avoids collisions with non-essential elements such as props within the level
...
@export_enum("Point by point", "expanding (WIP)") var bake_method: int
#--------------------------------------------------------------#
